...
| REQ-04.005 | El sistema no ocultará silenciosamente rutas undefined; deberá mostrar fallback visible. | `routeGenerate` cierra con `default: return null`, pero todas las rutas declaradas en `AppRoutes` están mapeadas en `switch`. | Riesgo leve |
...
| REQ-06.007 | El sistema ofrecerá una ruta de exportación PDF desde el detalle de propiedad. | `pagina_detalle_propiedad_pdf.dart` genera PDF con `printing` + `pdf`, pre-carga imágenes y usa `Printing.layoutPdf`. | En código |
...
## Validación de rutas (`07_routes`) vs páginas reales en `08_pantallas/`
Hallazgo directo: `routeGenerate` mapea todas las constantes de `AppRoutes` a páginas/widgets concretos. De ese mapeo, las páginas ubicadas en `08_pantallas/` confirmadas son:

| Ruta | Página/widget | En `08_pantallas/` |
|---|---|---|
| `/editaespacio` | `PaginaEditaEspacio` | `tu_cuenta/tus_espacios` |
| `/compraespacios` | `PaginaCompraEspacios` | `tu_cuenta/tus_espacios/compra_espacios` |
| `/perfil` | `PaginaPerfilWidget` | `08_pantallas/perfil` |
| `/localidades` | `PaginaBuscaLocalidadGMaps` | `08_pantallas/ubicacion` |
| `/listalocalidades` | `LocalidadesListScreen` | fuera de `08_pantallas` |
| `/mapapropiedades` | `PaginaMapaPropiedades` | `14_geolocalizacion` |
| `/listaspropiedades` | `PageMisListas` | `03_listas` |
| `/fotospropiedad`, `/fotospropiedadpaginada`, `/agregamultiplesfotos`, `/carouselfotospropiedad`, `/gestionfotopropiedad`, `/fotospropiedadminiaturas` | páginas de fotos | `22_imagenes` |

Rutas declaradas sin página visible en `08_pantallas/`: `generadatapropiedades`, `generadatausuarios`, `generadatapromotores`, `buscapropiedades`, `filtrapropiedades`, `buscalocalidad`, `fotosagregafotoalista`, `gestionfotoscasa`, `catalogotipotransaccion`, `opciones`. Se mantienen como riesgo en `03_SDD`.

## Cobertura explícita de `08_pantallas` subareas en EARS

| Subárea | EARS relacionados | Estado |
|---|---|---|
| `08_pantallas/inicio` | REQ-06.001 a REQ-06.009 | En código |
| `08_pantallas/propiedades` | REQ-06.006, REQ-06.007 | En código |
| `08_pantallas/perfil` | REQ-14.001 a REQ-14.005 | En código |
| `08_pantallas/ubicacion` | REQ-08.001 a REQ-08.006 | En código |
| `08_pantallas/tu_cuenta/tus_espacios` | REQ-12.001 a REQ-12.006 | En código |
| `08_pantallas/tu_cuenta/compra_espacios` | REQ-12.004, REQ-12.005 | En código |
| `08_pantallas/tu_cuenta/grupos` | REQ-10.001 a REQ-10.008 | En código |
| `08_pantallas/tu_cuenta/conocidos` | REQ-11.001 a REQ-11.005 | En código |
| `08_pantallas/widgets_comunes` | REQ-16.001 a REQ-16.005 | En código |

## PDF export — verificación directa
- Archivo: `lib/08_pantallas/propiedades/pagina_detalle_propiedad_pdf.dart`.
- Dependencias usadas: `pdf`, `printing`.
- Flujo: recibe `ValueEspaciosCasaGet`, prefoto principal, ids galería; descarga base64, limita galería a 12, genera PDF Letter con encabezado, descripción, precios, datos grid, ubicación, contacto y galería; entrega por `Printing.layoutPdf`.
- Evidencia: rutina `PdfGeneratorService.generarYDescargarPDF`, `_build*`, `Printing.layoutPdf`.
- Cierre: `REQ-06.007` se confirma como **En código** porque la generación y exportación están implementadas, no solo como stub.
