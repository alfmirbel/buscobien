...
---
## REQ-SOC-003 — Tus espacios y compra
**Ubicación:** `lib/08_pantallas/tu_cuenta/tus_espacios/`  
**Actor:** propietario/agente

- **REQ-SOC-003.1** Ubiquitous
  The system SHALL list user-published properties and purchased spaces.
- **REQ-SOC-003.2** Ubiquitous
  The system SHALL create/edit property records with conditional fields per property type.
- **REQ-SOC-003.3** Event-driven
  On purchase flow submit, The system SHALL persist purchase metadata and exposed totals.

## REQ-COMP-001 — Compra de espacios
**Ubicación:** `lib/08_pantallas/tu_cuenta/tus_espacios/compra_espacios/`  
**Actor:** usuario autenticado

- **REQ-COMP-001.1** Ubiquitous
  The system SHALL expose purchase flow with payment-method selection and totals.
- **REQ-COMP-001.2** Event-driven
  On purchase confirmation, The system SHALL write purchase document to CouchDB.

## REQ-TAB-001 — Catálogo/flujo Propiedades
**Ubicación:** `lib/08_pantallas/inicio/`, `lib/08_pantallas/propiedades/`  
**Actor:** usuario autenticado o anónimo

- **REQ-TAB-001.1** Ubiquitous
  The system SHALL present a paged property catalog from CouchDB with server-side skip/limit.
- **REQ-TAB-001.2** Event-driven
  On filter change, The system SHALL invalidate search providers and reload page 1.
- **REQ-TAB-001.3** Event-driven
  On detail/open, The system SHALL navigate to property detail and expose PDF export.

**Acceptance criteria**
- `FC-001` Page size is exactly 10 documents in pagination provider.
- `FC-002` PDF uses Printing.layoutPdf with preloaded base64 images.

## REQ-LST-004 — Gestión de listas
**Ubicación:** `lib/03_listas/`  
**Actor:** usuario autenticado

- **REQ-LST-004.1** Ubiquitous
  The system SHALL create, delete and reorder user lists, with "Favoritas" first.
- **REQ-LST-004.2** Ubiquitous
  The system SHALL add and remove properties from lists.
- **REQ-LST-004.3** State-driven
  While list becomes empty after remove, The system SHALL reflect empty state.

## REQ-LST-005 — Likes y favoritos
- **REQ-LST-005.1** Ubiquitous
  The system SHALL toggle me gusta with optimistic UI feedback.
- **REQ-LST-005.2** Event-driven
  When first like happens and "Favoritas" does not exist, The system SHALL create it.

## REQ-LST-006 — Compartir
**Ubicación:** `lib/03_listas/page_compartir_con_conocido.dart`, `page_compartir_con_grupo.dart`  
**Actor:** usuario autenticado

- **REQ-LST-006.1** State-driven
  While share target is known, The system SHALL allow sharing up to 5 known users or groups.
- **REQ-LST-006.2** Event-driven
  On confirm, The system SHALL persist share records and notify target.
- **REQ-LST-006.3** Unwanted
